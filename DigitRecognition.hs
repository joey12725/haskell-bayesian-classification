...
showPixelImage img = (intercalate "\n" [[ if x == False then ' ' else '#'| x <- row] | row <- img]) ++ "\n"
...
lookupVal key lst = do
    let lookUpList = [y | (x,y) <- lst, x == key]
    if length lookUpList == 1 then  head lookUpList else error "key not found or multiple values found"
    --if length lookUpList == 0 then error "Key not found"
...
buildCorpus imgLbls = [(x, [y | (y,z) <- imgLbls, z == x]) | x <- [0..9]]
...
probOfDigit corpus digit = 
    (length (lookupVal digit corpus)) `outOf` (length [x | (z,y) <- corpus, x <- y])
...
probOfFeature imgs ftr =
    (length ([x | x <- imgs, hasFeature x ftr])) `outOf` (length imgs)
...
probOfNoFeature imgs ftr = 
    (1 `outOf` 1) - (probOfFeature imgs ftr)
...
--    probOfDigit corpus digit * product [probOfFeature (snd (corpus!!x)) ftr | (x,y) <- zip [0..] newImg, y == True] * product [probOfNoFeature (snd (corpus!!x)) ftr | (x,y) <- zip [0..] newImg, y == False]
    probOfDigit corpus digit * product [probOfFeature (lookupVal digit corpus) x | (x,y) <- zip [0..] (concat newImg), y == True] * product [probOfNoFeature (lookupVal digit corpus) x | (x,y) <- zip [0..] (concat newImg), y == False]
...
--approximateRational :: Rational -> Double
--approximateRational x = (numerator x) / (denominator x)

--foo :: (Digit, Rational) -> Double
--foo (x,y) = approximateRational y

--maxRational :: [(Digit, Rational)] -> (Digit, Rational)
--maxRational lst = maximumBy (comparing foo) lst

classifyImage :: Corpus -> PixelImage -> Digit
classifyImage corpus newImg = do
    let rankList = [((rankOfDigit corpus x newImg), x) | x <- [0..9]]
    let (a,b) = maximum rankList
    b
